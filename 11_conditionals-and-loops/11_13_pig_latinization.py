...
# You'll need to use conditional statements to decide wen a word is over.
#
# For example: You would never guess --> ouyay ouldway evernay uessgay  

# Thoughts::
# Start with a for loop "char in story"
# if char is " ">>>start loop, with a variable named 'word', 
    # >>>withing loop:
    #   slice first_letter
    #   first_letter = (0,0) >>check this! either way it should relate to indexing
    #   aug = first_letter + to 'ay'
    #   end_word = word sans first_letter
    #   pl_word = end_word + aug
    #       >>>end loop with " "
    #   print pl_word
#to do:
    # start with clip 
        # once the program functions with clip, then try story 
# y = "You"
...
for char in story:
    ...
    if char.isalpha():
        ...
        for i in list(word):
            if i == word[0:1]:
                first_letter += i       
            elif i in list(word[1:-1:]): 
                end_word += i
        
        word = ""        
        pl_clip += (end_word + first_letter + "ay ")
print(pl_clip)
